...
const TagInput = ({
    ...
    return (
        <div>
            <div className='relative border cursor-pointer border-grey-dark p-4 rounded-[5px] text-sm' ref={wrapperRef}>

                <div className='flex flex-wrap items-center gap-4'>
                    {tags.map((tag: any, index: number) => (
                        <span key={index} className="py-[10px] border border-grey-dark items-center rounded-full px-4 w-max flex flex-wrap gap-[10px]">
                            <span>{tag}</span>
                            <span onClick={() => handleRemove(tag)}><CloseIcon /></span>
                        </span>
                    ))}
                    <input
                        type="text"
                        ...
                        className={`p- outline-none py-2 placeholder:text-placeholder ${tags.length === 0 ? 'w-full' : ''} disabled:bg-transparent`}
                    />
                ...
                {isFocused ? (
                    <div className="mt-2 -ml-4 border border-primary rounded-[5px] relatve z-10 absolute w-full bg-white overflow-hidden">
                        <ul>
                            {suggestions.filter((x: any) => !tags.includes(x)).map((v: any, i: number) => (
                                <li key={i}
