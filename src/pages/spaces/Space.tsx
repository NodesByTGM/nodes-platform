...
import { Back, SectionNavs } from "../../components";
import Discussion from "./space-sections/Discussion";
import About from "./space-sections/About";

import Members from "./space-sections/Members";
...
export default function Space() {
  ...
  return (
    <div>
      <Back link={"/spaces"} text="Go back" />

      <div className="mt-4 mb-8 flex gap-6 items-start p-6 rounded-[5px] border border-[#EFEFEF] bg-[#ffffff]  ">
        <div className="flex items-center justify-between w-full">
          <div className="flex gap-6">
            <div className="rounded-lg size-[80px] bg-[#D9D9D9]"></div>

            <div className="flex flex-col gap-2 text-[#212121] flex-1">
              <span className="font-medium text-[32px] ">Main Title</span>
              <div className="flex  gap-4">
                <div className="flex gap-2">
                  <PeopleEmoji />
                  <span className="text-xs font-normal">555 members</span>
                </div>
                <div className="flex gap-2">
                  <CommentsEmoji />
      ...
      <div className="mt-[56px]">
        {selectedNav?.label?.toLowerCase() == "discussion" && <Discussion />}

        {selectedNav?.label?.toLowerCase() == "about" && <About />}

        {selectedNav?.label?.toLowerCase() == "members" && <Members />}
      </div>
    </div>
  );
}
